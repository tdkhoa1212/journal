...
if opt.MFPT_data:
    ...
    # Outer rate ################################################
    OuterRaceFault_1 = divide_sample(scipy.io.loadmat(opt.data_dir + '2 - Three Outer Race Fault Conditions/OuterRaceFault_1.mat')['bearing'][0][0][2])
    OuterRaceFault_2 = divide_sample(scipy.io.loadmat(opt.data_dir + '2 - Three Outer Race Fault Conditions/OuterRaceFault_2.mat')['bearing'][0][0][2])
    OuterRaceFault_3 = divide_sample(scipy.io.loadmat(opt.data_dir + '2 - Three Outer Race Fault Conditions/OuterRaceFault_3.mat')['bearing'][0][0][2])
    OuterRaceFault_vload_1 = divide_sample(scipy.io.loadmat(opt.data_dir + '3 - Seven More Outer Race Fault Conditions/OuterRaceFault_vload_1.mat')['bearing'][0][0][2])
    OuterRaceFault_vload_2 = divide_sample(scipy.io.loadmat(opt.data_dir + '3 - Seven More Outer Race Fault Conditions/OuterRaceFault_vload_2.mat')['bearing'][0][0][2])
    OuterRaceFault_vload_3 = divide_sample(scipy.io.loadmat(opt.data_dir + '3 - Seven More Outer Race Fault Conditions/OuterRaceFault_vload_3.mat')['bearing'][0][0][2])
    OuterRaceFault_vload_4 = divide_sample(scipy.io.loadmat(opt.data_dir + '3 - Seven More Outer Race Fault Conditions/OuterRaceFault_vload_4.mat')['bearing'][0][0][2])
    OuterRaceFault_vload_5 = divide_sample(scipy.io.loadmat(opt.data_dir + '3 - Seven More Outer Race Fault Conditions/OuterRaceFault_vload_5.mat')['bearing'][0][0][2])
    OuterRaceFault_vload_6 = divide_sample(scipy.io.loadmat(opt.data_dir + '3 - Seven More Outer Race Fault Conditions/OuterRaceFault_vload_6.mat')['bearing'][0][0][2])
    OuterRaceFault_vload_7 = divide_sample(scipy.io.loadmat(opt.data_dir + '3 - Seven More Outer Race Fault Conditions/OuterRaceFault_vload_7.mat')['bearing'][0][0][2])
    ...
    # Inter rate ###################################################################################################################################
    InnerRaceFault_vload_1 = divide_sample(scipy.io.loadmat(opt.data_dir + '4 - Seven Inner Race Fault Conditions/InnerRaceFault_vload_1.mat')['bearing'][0][0][2])
    InnerRaceFault_vload_2 = divide_sample(scipy.io.loadmat(opt.data_dir + '4 - Seven Inner Race Fault Conditions/InnerRaceFault_vload_2.mat')['bearing'][0][0][2])
    InnerRaceFault_vload_3 = divide_sample(scipy.io.loadmat(opt.data_dir + '4 - Seven Inner Race Fault Conditions/InnerRaceFault_vload_3.mat')['bearing'][0][0][2])
    InnerRaceFault_vload_4 = divide_sample(scipy.io.loadmat(opt.data_dir + '4 - Seven Inner Race Fault Conditions/InnerRaceFault_vload_4.mat')['bearing'][0][0][2])
    InnerRaceFault_vload_5 = divide_sample(scipy.io.loadmat(opt.data_dir + '4 - Seven Inner Race Fault Conditions/InnerRaceFault_vload_5.mat')['bearing'][0][0][2])
    InnerRaceFault_vload_6 = divide_sample(scipy.io.loadmat(opt.data_dir + '4 - Seven Inner Race Fault Conditions/InnerRaceFault_vload_6.mat')['bearing'][0][0][2])
    InnerRaceFault_vload_7 = divide_sample(scipy.io.loadmat(opt.data_dir + '4 - Seven Inner Race Fault Conditions/InnerRaceFault_vload_7.mat')['bearing'][0][0][2])
...
if opt.PU_data_table_8:
    ...
    if load:
        Healthy_train = np.load(opt.data_dir + 'Healthy_train.npy')
        Healthy_train_label = np.load(opt.data_dir + 'Healthy_train_label.npy')
    else:
        Healthy_train = K002[:, :min_]
        Healthy_train_label = one_hot(0, 3)
        with open(opt.data_dir + 'Healthy_train.npy', 'wb') as f:
            np.save(f, Healthy_train)
        with open(opt.data_dir + 'Healthy_train_label.npy', 'wb') as f:
            np.save(f, Healthy_train_label)

    if load:
        OR_Damage_train = np.load(opt.data_dir + 'OR_Damage_train.npy')
        OR_Damage_train_label = np.load(opt.data_dir + 'OR_Damage_train_label.npy')
    else:
        OR_Damage_train = np.concatenate((KA01[:, :min_], KA05[:, :min_], KA07[:, :min_]))
        OR_Damage_train_label = one_hot(1, 3)
        with open(opt.data_dir + 'OR_Damage_train.npy', 'wb') as f:
            np.save(f, OR_Damage_train)
        with open(opt.data_dir + 'OR_Damage_train_label.npy', 'wb') as f:
    ...
    if load:
      # Adding noise to IR training data---------------------------------------------------------------------------
        ...
        IR_Damage_train = np.load(opt.data_dir + 'IR_Damage_train.npy')
        IR_Damage_train_label = np.load(opt.data_dir + 'IR_Damage_train_label.npy')
    else:
        IR_Damage_train = np.concatenate((KI01[:, :min_], KI05[:, :min_], KI07[:, :min_]))
        IR_Damage_train_label = one_hot(2, 3)
        with open(opt.data_dir + 'IR_Damage_train.npy', 'wb') as f:
            np.save(f, IR_Damage_train)
        with open(opt.data_dir + 'IR_Damage_train_label.npy', 'wb') as f:
            np.save(f, IR_Damage_train_label)
    
    if load:
        Healthy_test = np.load(opt.data_dir + 'Healthy_test.npy')
        Healthy_test_label = np.load(opt.data_dir + 'Healthy_test_label.npy')
    else:
        Healthy_test = K001[:, :min_]
        Healthy_test_label = one_hot(0, 3)
        with open(opt.data_dir + 'Healthy_test.npy', 'wb') as f:
            np.save(f, Healthy_test)
        with open(opt.data_dir + 'Healthy_test_label.npy', 'wb') as f:
            np.save(f, Healthy_test_label)
    
    if load:
        OR_Damage_test = np.load(opt.data_dir + 'OR_Damage_test.npy')
        OR_Damage_test_label = np.load(opt.data_dir + 'OR_Damage_test_label.npy')
    else:
        OR_Damage_test = np.concatenate((KA22[:, :min_], KA04[:, :min_], KA15[:, :min_], KA30[:, :min_], KA16[:, :min_]))
        OR_Damage_test_label = one_hot(1, 3)
        with open(opt.data_dir + 'OR_Damage_test.npy', 'wb') as f:
            np.save(f, OR_Damage_test)
        with open(opt.data_dir + 'OR_Damage_test_label.npy', 'wb') as f:
            np.save(f, OR_Damage_test_label)

    if load:
        IR_Damage_test = np.load(opt.data_dir + 'IR_Damage_test.npy')
        IR_Damage_test_label = np.load(opt.data_dir + 'IR_Damage_test_label.npy')
    else:  
        IR_Damage_test = np.concatenate((KI14[:, :min_], KI21[:, :min_], KI17[:, :min_], KI18[:, :min_], KI16[:, :min_]))
        IR_Damage_test_label = one_hot(2, 3)
        with open(opt.data_dir + 'IR_Damage_test.npy', 'wb') as f:
            np.save(f, IR_Damage_test)
        with open(opt.data_dir + 'IR_Damage_test_label.npy', 'wb') as f:
